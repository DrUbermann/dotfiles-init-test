...
script="${TMPDIR:-/tmp}/chezmoi-install_$$.tmp"
...
ssl_get https://get.chezmoi.io | sed '$d' > /tmp/chezmoi-install.sh
cat >> /tmp/chezmoi-install.sh <<'EOF'
...
main "${@}"
EOF
sh /tmp/chezmoi-install.sh init --apply https://github.com/Owner/Repo.git